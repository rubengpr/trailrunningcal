...
interface EventShareWhatsappButtonProps {
  message: string;
  label: string;
  eventId: string;
  eventSlug: string;
...
export function EventShareWhatsappButton({
  message,
  label,
  eventId,
  eventSlug,
  iconOnly = false,
  className = '',
}: EventShareWhatsappButtonProps) {
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`;

  const handleClick = () => {
    track(ANALYTICS_EVENTS.EVENT_SHARE_CLICKED, {
      event_id: eventId,
