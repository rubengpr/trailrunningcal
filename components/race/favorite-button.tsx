...
export function FavoriteButton({ isFavorited, onToggle, label, iconOnly = false, className = '', title }: FavoriteButtonProps) {
  ...
  return (
    <button
      onClick={handleClick}
      title={title ?? label}
      className={`flex items-center justify-center gap-1.5 rounded-md font-medium border transition-colors cursor-pointer whitespace-nowrap ${iconOnly ? 'p-2' : 'px-4 py-2'} ${className} ${
        isFavorited
