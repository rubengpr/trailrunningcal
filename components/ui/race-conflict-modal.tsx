...
export function RaceConflictModal({ isOpen, onClose, conflicts }: RaceConflictModalProps) {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title={t('title')} maxWidth="2xl">
      ...
      <ul className="mb-6 divide-y divide-gray-100 rounded-md border border-gray-200">
        {conflicts.map((race) => (
          <li key={race.id} className={`grid items-center gap-4 px-4 py-3 ${showUrl ? 'grid-cols-[1fr_auto_1fr]' : 'grid-cols-[1fr_auto]'}`}>
