...
export interface PendingQueueResult {
  added: PendingQueueEntry[];
  skipped: SkippedQueueEntry[];
}

interface AdminPendingQueueContentProps {
  entries: PendingQueueEntry[];
  namespace: Parameters<typeof useTranslations>[0];
  addEntries: (urls: string[]) => Promise<PendingQueueResult>;
  deleteEntry: (id: string) => Promise<void>;
}

export function AdminPendingQueueContent({
  entries,
  namespace,
  addEntries,
  deleteEntry,
}: AdminPendingQueueContentProps) {
  const t = useTranslations(namespace);
  const locale = useLocale();

  const [urlsText, setUrlsText] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [pendingEntries, setPendingEntries] = useState<PendingQueueEntry[]>(entries);
  const [skippedEntries, setSkippedEntries] = useState<SkippedQueueEntry[]>([]);
  const [entryToDelete, setEntryToDelete] = useState<PendingQueueEntry | null>(null);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {pendingEntries.length === 0 ? (
        ...
        <Table>
          <TableHeader>
            <TableCell header>{t('table.url')}</TableCell>
            <TableCell header>{t('table.addedAt')}</TableCell>
            <TableCell header />
          </TableHeader>
          <TableBody>
