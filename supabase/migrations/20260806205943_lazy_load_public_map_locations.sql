...
create or replace function public.get_public_event_locations(
  p_locations jsonb
)
returns table(
  city text,
...
as $function$
  with requested_rows as (
    select
      btrim(requested.city) as city,
      btrim(requested.province) as province
    from jsonb_to_recordset(
      case
        when jsonb_typeof(p_locations) = 'array' then p_locations
        else '[]'::jsonb
      end
    ) as requested(city text, province text)
    where requested.city is not null
      and requested.province is not null
    limit 500
  ),
  requested_locations as (
  ...
  select
    ...
    location.latitude,
    location.longitude
  from requested_locations requested
  join public.city_locations location
    on location.city = requested.city
    and location.province = requested.province
  order by location.city, location.province;
$function$;

revoke all on function public.get_public_event_locations(jsonb) from public;

grant execute on function public.get_public_event_locations(jsonb)
