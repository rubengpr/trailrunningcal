...
begin
  ...
  if p_sort_column not in ('dates', 'name') then
    raise exception 'p_sort_column must be dates or name'
      using errcode = '22023';
  end if;
  ...
  return query
  -- The admin table shows the date range of the edition the UI considers
  -- relevant (the upcoming one, or the most recent past one), so ordering must
  -- use that same edition instead of the event's overall earliest race date.
  with race_anchor as (
    select
      race.event_id,
      coalesce(
