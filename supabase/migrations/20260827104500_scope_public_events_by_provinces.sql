-- Scope the public events page by a set of provinces so autonomous community
-- pages can list every race in their provinces with a single call.
-- The previous signature is dropped: adding a defaulted parameter would create
-- an overload and leave named-argument calls ambiguous.
drop function if exists public.get_public_events_page(
  date, integer, smallint[], text[], text[], text[], text, text, boolean
);
...
as $function$
  with upcoming_dates as (
    select r.event_id, min(r.date) as start_date
    from public.races r
    ...
    select
      ...
      array_remove(
        array[
          ...
          case
            when lower(concat_ws(' ', e.name, r.name)) like '%vertical%'
              or lower(concat_ws(' ', e.name, r.name)) like '%vertik%'
              or lower(concat_ws(' ', e.name, r.name)) ~ '(^|[^[:alnum:]])kmv([^[:alnum:]]|$)'
              or lower(concat_ws(' ', e.name, r.name)) ~ '(^|[^[:alnum:]])kv([^[:alnum:]]|$)|kv$'
              or (
                r.distance_km <= 8
                and r.elevation_gain_m >= 500
                and r.elevation_gain_m::numeric / nullif(r.distance_km, 0) >= 100
              ) then 'km-vertical'
          end,
          case when r.distance_km >= 50 then 'ultra-trail' end,
          case when r.distance_km >= 40 and r.distance_km < 50 then 'maraton' end,
          case when r.distance_km >= 20 and r.distance_km <= 24 then 'media-maraton' end
        ],
        null
      ) as categories
    from public.races r
  ...
  event_summaries as (
    select e.id, e.name, e.slug, u.start_date, max(rr.date) as end_date
  ...
  filtered_events as (
    ...
    where (
      ...
      and (
        ...
        or exists (
          select 1 from relevant_races rr
          where rr.event_id = es.id and rr.province = p_scope_province
        )
      )
      and (
        ...
        or exists (
          select 1 from relevant_races rr
          where rr.event_id = es.id and rr.province = any(p_scope_provinces)
        )
      )
      and (
        ...
        or exists (
          select 1 from relevant_races rr
          where rr.event_id = es.id and rr.province = any(p_provinces)
        )
      )
      and (
      ...
      and (
        ...
        or exists (
          select 1 from relevant_races rr
          where rr.event_id = es.id and rr.categories && p_race_types
        )
      )
      and (
        ...
        or exists (
          select 1 from relevant_races rr
          where rr.event_id = es.id and p_scope_race_type = any(rr.categories)
        )
      )
  ),
  counted_events as (
    select fe.*, count(*) over () as total_count
  ...
  group by pe.id, pe.name, pe.slug, pe.start_date, pe.end_date, pe.total_count
  order by pe.start_date, pe.name, pe.id;
$function$;

revoke all on function public.get_public_events_page(
  date, integer, smallint[], text[], text[], text[], text, text[], text, boolean
) from public;
